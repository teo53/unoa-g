...
class _CreatorAnalyticsScreenState
  ...
  // Mock data
  final _mockStats = const _CreatorStats(
  ...
  final List<_WeeklyData> _weeklyData = [
    _WeeklyData('월', 45, 12),
    _WeeklyData('화', 62, 18),
    _WeeklyData('수', 38, 8),
    _WeeklyData('목', 71, 22),
  ...
  final List<_PopularMessage> _popularMessages = [
      ...
      body: SafeArea(
        child: CustomScrollView(
          slivers: [
            ...
            // Message stats
            SliverToBoxAdapter(
              child: _buildMessageStats(isDark),
            ),

            // Popular messages
            SliverToBoxAdapter(
              child: _buildPopularMessages(isDark),
            ),

            // Demo mode indicator
            if (isDemoMode)
              SliverToBoxAdapter(
                child: _buildDemoIndicator(isDark),
              ),
      ...
      child: Row(
        children: [
          Expanded(
            child: _StatCard(
              title: '구독자',
              value: _formatNumber(_mockStats.totalSubscribers),
              change: '+${_mockStats.subscriberGrowth}',
              changeLabel: '오늘',
          ...
          Expanded(
            child: _StatCard(
              title: '수익',
              value: '${_formatNumber(_mockStats.totalDtEarned)} DT',
              change: '+${_formatNumber(_mockStats.weeklyDtEarned)}',
              changeLabel: '이번 주',
          ...
          SizedBox(
            ...
            child: Row(
              ...
              children: _weeklyData.map((data) {
                final height = (data.messages / maxMessages) * 120;
                return Expanded(
                  child: Column(
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _StatRow(
            label: '받은 답장',
            value: '${_formatNumber(_mockStats.totalMessages)}개',
            isDark: isDark,
          ),
          const SizedBox(height: 12),
          _StatRow(
            label: '도네이션 메시지',
            value: '${_mockStats.donationMessages}개',
            isDark: isDark,
            highlight: true,
          ),
          const SizedBox(height: 12),
...
class _CreatorStats {
  ...
  const _CreatorStats({
    ...
    required this.subscriberGrowth,
    required this.totalMessages,
    required this.donationMessages,
    required this.totalDtEarned,
    required this.weeklyDtEarned,
